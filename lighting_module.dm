#define	DAY_LENGTH	4500
#define NIGHT_LENGTH 4500
#define RAIN_CHANCE 25

#define HOUR_LENGTH 750
#define MINUTE_LENGTH 10
	...
	proc
		...
		HourLoop()
			while (1)
				...
				for ( var/mob/animal/monkey/monkey in world.contents )
					monkey.HourTick()
		...
		MinuteLoop()
			while (1)
				sleep(MINUTE_LENGTH)
				for(var/obj/item/trap/T in world.contents)
					if (T:ready == 1)
						T:Check()


		setNight()
			world << "<B>The sun sets."
