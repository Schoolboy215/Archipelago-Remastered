			...
			if ( ownerName == src.name)
				...
				spawn(DESTROY_OTHER_TIME)
					...
					if ( prob(chance) )
						...
						usr:Hurt(rand(10,20),"headbutts [src] in frustration, and is killed.")
			...
			verb
				Set_Password()
					...
					if ( ownerName!= usr.name)
						usr << "<font color = red>This isn't your door"
						return
					var/response
					response = input("What do you want the password to be?(Enter blank to turn off password)") as null|text
					if (response == null)
						//password = ""
						if (password != "")
							usr << "<font color = green>That door's password is still : <b>[password]"
						else
							usr << "<font color = green>That door has no password"

						return
					if (response == "")
						password = ""
						usr << "<font color = green>That door can now be opened by anyone"
						return
					else
						password = response
						usr << "<font color = green>You just set that door's password to : <b>[password]"
				/*AllowTribe()
					set src in view(1,usr)
					if ( ownerName != usr.name )
			...
			DblClick() // Interact with door
				...
				1)
					if ( state )
						...
						if (password != "")
							var/response = input("Enter password (case sensitive)") as null|text
							if (response == null)
								return
							if (response != password)
								usr << "<font color = red> Incorrect!"
								return
						icon_state = open_icon
						density = 0
						state = 1
