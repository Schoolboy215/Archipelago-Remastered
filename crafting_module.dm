...
obj/item/misc/Brick_Mold
	...
	proc
		addClay(obj/newclay,mob/player/owner)
			...
			usr.lastaction = "filling : Brick Mold"
			owner.setBusy(1)
			sleep(MOLD_TIME)
			if ( !owner )	return
			owner.setBusy(0)
			...
			usr.lastaction = "filled : BrickMold"
			newclay.Move(src)
...
mob/player/proc
	...
	DoSandCrafting(obj/item/tool/Blow_Pipe/pipe,obj/Fire/fire)
		...
		chance = (getSkillLevel(src.XP_Crafting)*10)
...
obj/item/tool/Hammer/MouseDrop(obj/item/misc/Rock/stone)
	...
	usr:Public_message("[usr] starts chipping away at a rock.",MESSAGE_CRAFTING)
	usr.lastaction = "hammering : Rock"
	...
	if ( prob(chance) )
		...
		usr.lastaction = "hammered : Stone Block"
		...
		usr.lastaction = "broke : Stone Block"
...
obj/item/misc/Soft_Clay_Bowl/MouseDrop(obj/fire)
	...
	usr.lastaction = "baking : Clay Bowl"
	...
	if ( prob(chance) )
		...
		usr.lastaction = "baked : Clay Bowl"
		...
		usr.lastaction = "broke : Clay Bowl"
...
obj/item/misc/Sand/MouseDrop(obj/item/tool/Blow_Pipe/pipe)
	...
	usr.lastaction = "filled : Blow Pipe"
	del src
...
/obj/item/misc/Cotton/MouseDrop(obj/building/Spinning_Wheel/wheel)
	...
	usr.lastaction = "spinning : Cotton"
	...
	if ( prob(chance) )
		...
		usr.lastaction = "failed to spin twine"
