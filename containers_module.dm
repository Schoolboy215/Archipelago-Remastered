	...
	vial
		...
		Drink()
			...
			if ( !item || !item2 )
				...
				return
			usr:DrinkPotion(item,item2,mixed,botheff)
			del item
			del item2
			item = null
			item2 = null
		...
		SetIcon()
			...
			if ( item )
				AdjustIconColor(liquid,item:color)
			if ( item2 )
				AdjustIconColor(liquid,item2:color)
