...
proc/gameMessage(mob/target,mes,mesType)
	if ( mesType )
		var color = getMessageColor(mesType)
		var mesName = getMessageName(mesType)
		target << "<FONT COLOR=[color]><B>\[Game - [mesName]]</B> [mes]"
	else
		target << mes
proc
	getMessageColor(mesType)
		switch (mesType)
			...
			if ( MESSAGE_SMOKING )		return "black"
			if ( MESSAGE_CARDS )		return "blue"
			if ( MESSAGE_TRAPPING )		return "green"
	...
	getMessageName(mesType)
		switch (mesType)
			...
			if ( MESSAGE_SMOKING )		return "Smoking"
			if ( MESSAGE_CARDS )		return "Cards"
			if ( MESSAGE_TRAPPING )		return "Trapping"
