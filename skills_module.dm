...
proc
	GetSkillName(skill)
		switch ( skill )
			...
			if ( SKILL_TRAPPING )	return "Trapping"
	AdjustChance(chance)
		if ( chance < 5 )
			chance = 5
		if ( chance > 95 )
	...
	proc
		StatSkills()
			...
			for ( skill = 1, skill <= MAX_SKILLS, skill++  )
				if ((usr:Effect_Type == 7) && (usr:Effect2_Type <> 3))
					statpanel("Skills",GetSkillName(skill),"[GetSkill(skill)]     [getXPLeft(GetSkillXP(skill))] XP remaining")
				if ((usr:Effect2_Type == 7) && (usr:Effect_Type <> 3))
					statpanel("Skills",GetSkillName(skill),"[GetSkill(skill)]     [getXPLeft(GetSkillXP(skill))] XP remaining")

				if ((usr:Effect_Type == 3) && (usr:Effect2_Type <> 7))
					statpanel("Skills",GetSkillName(skill),"[GetSkill(skill)]     [getXPLeft(GetSkillXP(skill))] XP remaining")
				if ((usr:Effect2_Type == 3) && (usr:Effect_Type <> 7))
					statpanel("Skills",GetSkillName(skill),"[GetSkill(skill)]     [getXPLeft(GetSkillXP(skill))] XP remaining")
				...
				if ((usr:Effect_Type <> 3 || 7) && (usr:Effect2_Type <> 3 || 7))
					statpanel("Skills",GetSkillName(skill),"[GetSkill(skill)]     [getXPLeft(GetSkillXP(skill))] XP remaining")
		...
		GetSkillXP(skill)
			switch ( skill )
				...
				if ( SKILL_TRAPPING )	return XP_Trapping
				else world.log << "Bad Skill number: [skill]"
			return 0

		getXPLeft(XP)
			if ( XP < SKILL_XP_LEVEL_2 )
				return (SKILL_XP_LEVEL_2 - XP)
			if ( XP < SKILL_XP_LEVEL_3 )
				return (SKILL_XP_LEVEL_3 - XP)
			if ( XP < SKILL_XP_LEVEL_4 )
				return (SKILL_XP_LEVEL_4 - XP)
			if ( XP < SKILL_XP_LEVEL_5 )
				return (SKILL_XP_LEVEL_5 - XP)
			if ( XP < SKILL_XP_LEVEL_6 )
				return (SKILL_XP_LEVEL_6 - XP)
			if ( XP < SKILL_XP_LEVEL_7 )
				return (SKILL_XP_LEVEL_7 - XP)
			if ( XP < SKILL_XP_LEVEL_8 )
				return (SKILL_XP_LEVEL_8 - XP)
			if ( XP < SKILL_XP_LEVEL_9 )
				return (SKILL_XP_LEVEL_9 - XP)
			if ( XP < SKILL_XP_LEVEL_10 )
				return (SKILL_XP_LEVEL_10 - XP)
		...
		AddXP(skill,xp)
			switch ( skill )
				...
				if ( SKILL_TRAPPING )	 XP_Trapping += xp
