...
obj
	Fire
		...
		New(l,maker)
			if (usr.loc.name == "Water")
				usr << "<font color = red>You cannot light fires on water!"
				del(src)
				return ..()
			Public_message("[maker] lights a campfire.",MESSAGE_FIRE)
			usr.lastaction = "lit : Campfire"
			SetLumin()
		...
		DblClick()
			switch (woodLeft)
				if ( 4 to MAX_CAMPFIRE_WOOD )	gameMessage(usr, "The fire is burning brightly.",MESSAGE_FIRE)
				if ( 3 )		gameMessage(usr, "The fire could use some more wood.",MESSAGE_FIRE)
				if ( 2 )		gameMessage(usr,"The fire is burning weakly.",MESSAGE_FIRE)
				if ( 1 )		gameMessage(usr, "The fire is about to burn out.",MESSAGE_FIRE)
		...
		proc
			SetLumin()
				var newLum
				switch (woodLeft)
					if (3 to MAX_CAMPFIRE_WOOD)	newLum = 4
					if (2)			newLum = 3
					if (1)			newLum = 2
	...
	item/misc
		MouseDrop(obj/fire)
			...
			if ( fire:addWood(burnable) )
				gameMessage(usr,"The [fire] already has plenty of fuel",MESSAGE_FIRE)
				return

			usr.Public_message("[usr] adds [src] to the [fire].",MESSAGE_FIRE)
			usr.lastaction = "added [src] to [fire]"
